...
export default function SourcesPage() {
  ...
  const [form, setForm] = useState({ name: "", url: "", source_type: "webpage" });
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const source = await sourcesApi.create(form);
      setSources((prev) => [source, ...prev]);
      setShowForm(false);
      setForm({ name: "", url: "", source_type: "webpage" });
    } catch (err: any) {
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">添加要每日抓取的网页或 RSS 订阅</p>
        </div>
        <button
              ...
              <div className="bg-background border border-border rounded-lg px-4 py-3 flex items-center gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {!source.is_active && (
                      <span className="px-1.5 py-0.5 text-xs bg-muted text-muted-foreground rounded">已禁用</span>
                    )}
                  </div>
